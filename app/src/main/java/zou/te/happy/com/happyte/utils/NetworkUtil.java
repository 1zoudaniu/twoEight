...
public class NetworkUtil {
    ...
    public static boolean printNetworkInfo(Context context) {
        ...
        if (connectivity != null) {
            ...
            Log.d(TAG, "-------------$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$-------------");
            Log.d(TAG, "getActiveNetworkInfo: " + in);
            NetworkInfo[] info = connectivity.getAllNetworkInfo();
            if (info != null) {
                for (int i = 0; i < info.length; i++) {
                    // if (info[i].getType() == ConnectivityManager.TYPE_WIFI) {
                    Log.d(TAG, "NetworkInfo[" + i + "]isAvailable : " + info[i].isAvailable());
                    Log.d(TAG, "NetworkInfo[" + i + "]isConnected : " + info[i].isConnected());
                    Log.d(TAG, "NetworkInfo[" + i + "]isConnectedOrConnecting : " + info[i].isConnectedOrConnecting());
                    Log.d(TAG, "NetworkInfo[" + i + "]: " + info[i]);
                    // }
                }
                Log.d(TAG, "\n");
            } else {
                Log.d(TAG, "getAllNetworkInfo is null");
            }
        }
        return false;
    }
    ...
    public static int getConnectedTypeINT(Context context) {
        ...
        if (net != null) {
            //if (Log.isPrint) {
            Log.d(TAG, "NetworkInfo: " + net.toString());
            //}
            return net.getType();
        }
        return -1;
